...
2. **Write `skills/<name>/SKILL.md`** from CONTRIBUTING.md's template. The frontmatter `description` is the only registry Claude reads: state both what the skill does and the concrete triggers for reaching for it, and name the neighbouring skills it defers to.
