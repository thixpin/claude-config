...
5. **Audit secrets and IAM** — no plaintext secrets in tracked files; credential variables marked `sensitive = true` (masks output, not state); secrets via secret-manager reference, never literals — and since secret values can still land in state, keep them out where possible and restrict state-backend access; no wildcard `Action: "*"` / `Resource: "*"` IAM without explicit justification. Deeper security analysis belongs to `security-audit`.
